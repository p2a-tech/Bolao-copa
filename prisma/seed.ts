...
import { seedTenantUsers } from "./seed-users";
import { seedDemoPredictions } from "./seed-demo-predictions";
...
async function main() {
  ...
  const demoTenant = await prisma.tenant.create({
    data: {
      ...
      landingSubtitle: "Palpite em todos os jogos, dispute o ranking e concorra a prêmios.",
  ...
  await prisma.sponsor.create({
    data: {
      ...
      logoUrl: "https://placehold.co/1200x150/00875A/FFFFFF/png?text=Patrocinador+Master+do+Bolao",
    ...
    [
      ...
      prisma.sponsor.create({
        data: {
          tenantId: demoTenant.id,
          name: name as string,
          logoUrl: `https://placehold.co/600x90/${bg}/${fg}/png?text=${encodeURIComponent(name as string)}`,
          linkUrl: "https://example.com",
          placement: "match",
        },
      })
  ...
  const usedEmails = new Set<string>([
    "super@bolao.com", "admin@bolao.com", "maria@exemplo.com", "admin@lacerda.com",
  ]);
  const usedCpfs = new Set<string>([
    "52998224725", "39053344705", "11144477735", "12345678909",
  ]);

  console.log("Populando 1000 usuários no tenant 'demo'...");
  const demoUsers = await seedTenantUsers(
    prisma, demoTenant.id, "demo", 1000, sharedPasswordHash, usedEmails, usedCpfs
  );
  console.log(`  ${demoUsers} usuários adicionados em 'demo'.`);

  console.log("Criando 10 palpites de exemplo para maria@exemplo.com...");
  const mariaPredictions = await seedDemoPredictions(
    prisma,
    "maria@exemplo.com",
    demoTenant.id
  );
  console.log(`  ${mariaPredictions} palpites criados (todos os status).`);
  ...
  const lacerdaTenant = await prisma.tenant.create({
    data: {
      ...
      landingSubtitle: "Palpite em todos os jogos da Copa, dispute o ranking ao vivo e concorra a prêmios com a galera que segue o @lacerdareporter.",
      welcomeMessage: "Bem-vindo ao Bolão do Lacerda! Boa sorte e bons palpites.",
    },
  });
  ...
  await prisma.sponsor.create({
    data: {
      ...
      logoUrl: "https://placehold.co/1200x150/00875A/FFFFFF/png?text=Patrocinador+Master",
      linkUrl: "https://example.com",
      placement: "global",
    },
  });
  ...
  console.log("Populando 1000 usuários no tenant 'lacerda'...");
  const lacerdaUsers = await seedTenantUsers(
    prisma, lacerdaTenant.id, "lacerda", 1000, sharedPasswordHash, usedEmails, usedCpfs
  );
  console.log(`  ${lacerdaUsers} usuários adicionados em 'lacerda'.`);

  const total = await prisma.match.count();
  console.log(`Pronto! ${TEAMS.length} selecoes e ${total} jogos criados.`);
  console.log("Super: super@bolao.com / super123 -> /superadmin");
  console.log("Tenant 'demo' (/demo/login):");
  console.log("  Admin:   admin@bolao.com / admin123");
  console.log("  Usuario: maria@exemplo.com / demo123");
